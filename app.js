...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (planEditorForm) {
        planEditorForm.addEventListener("submit", handlePlanEditorSubmit);
    }

    const ottEditorForm = document.getElementById("ottEditorForm");
    if (ottEditorForm) {
        ottEditorForm.addEventListener("submit", handleOttEditorSubmit);
    }

    const rechargeSearch = document.getElementById("rechargeSearch");
    if (rechargeSearch) {
        rechargeSearch.addEventListener("input", filterRechargesLog);
    }
...
function initLocalStorage() {
    ...
    if (!localStorage.getItem("arb_admin_user")) {
        localStorage.setItem("arb_admin_user", "admin");
    }
    if (!localStorage.getItem("arb_admin_pass")) {
        localStorage.setItem("arb_admin_pass", "admin123");
    }
}
...
function handleAdminLogin(e) {
    ...
    const storedUser = localStorage.getItem("arb_admin_user") || "admin";
    const storedPass = localStorage.getItem("arb_admin_pass") || "admin123";
...
function switchDashTab(tabName) {
    const btnRecharges = document.getElementById("btn-tab-recharges");
    const btnPlans = document.getElementById("btn-tab-plans");
    const btnOtt = document.getElementById("btn-tab-ott");
    const btnMessages = document.getElementById("btn-tab-messages");
    const btnSettings = document.getElementById("btn-tab-settings");
    
    const tabs = [btnRecharges, btnPlans, btnOtt, btnMessages, btnSettings];
    tabs.forEach(btn => { if (btn) btn.classList.remove("active"); });
    
    const dashRecharges = document.getElementById("dash-recharges-tab");
    const dashPlans = document.getElementById("dash-plans-tab");
    const dashOtt = document.getElementById("dash-ott-tab");
    const dashMessages = document.getElementById("dash-messages-tab");
    const dashSettings = document.getElementById("dash-settings-tab");

    if (dashRecharges) dashRecharges.classList.remove("active");
    if (dashPlans) dashPlans.classList.remove("active");
    if (dashOtt) dashOtt.classList.remove("active");
    if (dashMessages) dashMessages.classList.remove("active");
    if (dashSettings) dashSettings.classList.remove("active");
    
    if (tabName === "recharges") {
        if (btnRecharges) btnRecharges.classList.add("active");
        if (dashRecharges) dashRecharges.classList.add("active");
        renderRechargeLogs();
    } else if (tabName === "plans") {
        if (btnPlans) btnPlans.classList.add("active");
        if (dashPlans) dashPlans.classList.add("active");
        renderAdminPlansList();
    } else if (tabName === "ott") {
        if (btnOtt) btnOtt.classList.add("active");
        if (dashOtt) dashOtt.classList.add("active");
        renderAdminOttPlansList();
    } else if (tabName === "messages") {
        if (btnMessages) btnMessages.classList.add("active");
        if (dashMessages) dashMessages.classList.add("active");
        renderContactMessages();
    } else if (tabName === "settings") {
        if (btnSettings) btnSettings.classList.add("active");
        if (dashSettings) dashSettings.classList.add("active");
        populateSettingsForm();
    }
}

// ==========================================================================
// Owner Dashboard Tab 1: Customer Recharges Log View
// ==========================================================================

function renderRechargeLogs(filteredLogs = null) {
    const tableBody = document.getElementById("rechargeTableBody");
    if (!tableBody) return;
    
    tableBody.innerHTML = "";
    
    const dataset = filteredLogs || recharges;
    
    if (dataset.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="10" style="text-align:center; padding: 20px;">No customer recharge logs found.</td></tr>`;
        return;
    }
    ...
    dataset.forEach((log) => {
        ...
        if (log.status === "Success") badgeClass = "status-success";
        if (log.status === "Failed") badgeClass = "status-failed";

        const isOtt = log.planCategory === "OTT";
        const typeBadge = isOtt 
            ? `<span class="plan-type-badge plan-type-ott"><i class="fas fa-tv"></i> OTT</span>`
            : `<span class="plan-type-badge plan-type-broadband"><i class="fas fa-wifi"></i> Fiber</span>`;
        
        const ottPreview = (log.ottApps && log.ottApps.length > 0)
            ? `<div style="font-size:10px; color:#be185d; margin-top:2px;">Apps: ${log.ottApps.slice(0, 3).join(", ")}${log.ottApps.length > 3 ? ` +${log.ottApps.length - 3} more` : ''}</div>`
            : "";

        const gstText = log.gstAmount ? `<div style="font-size:10px; color:#0284c7;">+ ₹${log.gstAmount} (18% GST)</div>` : '';

        row.innerHTML = `
            <td><small>${log.timestamp}</small></td>
            <td><strong>${log.custID}</strong></td>
            <td>${log.custName}</td>
            <td>
                <div>${log.custMobile}</div>
                <div style="font-size: 11px; color:#888;">${log.custEmail}</div>
            </td>
            <td>
                <div>${typeBadge} <strong>${log.planName}</strong></div>
                ${ottPreview}
            </td>
            <td>
                <span class="tenure-pill-log">${log.tenure || '1 Month'}</span>
            </td>
            <td>
                <strong style="color:var(--primary-color);">₹${log.planPrice}</strong>
                ${gstText}
            </td>
            <td><code style="background:#f1f5f9; padding: 2px 4px; border-radius:3px; font-size:11px;">${log.transactionID}</code></td>
            <td>
                <span class="status-badge ${badgeClass}" id="badge-${log.id}">${log.status.toUpperCase()}</span>
            </td>
            <td>
                <select class="status-select" onchange="updateRechargeStatus(${log.id}, this.value)">
                    <option value="Pending" ${log.status === "Pending" ? "selected" : ""}>Pending</option>
                    <option value="Success" ${log.status === "Success" ? "selected" : ""}>Approve (Success)</option>
                    <option value="Failed" ${log.status === "Failed" ? "selected" : ""}>Decline (Failed)</option>
                </select>
            </td>
        `;
...
function updateRechargeStatus(logId, newStatus) {
    const index = recharges.findIndex(item => item.id === logId);
    if (index === -1) return;
    
    recharges[index].status = newStatus;
    localStorage.setItem("arb_recharges", JSON.stringify(recharges));
    ...
    if (badge) {
        ...
        if (newStatus === "Success") badge.classList.add("status-success");
        else if (newStatus === "Failed") badge.classList.add("status-failed");
...
function filterRechargesLog() {
    const query = (document.getElementById("rechargeSearch")?.value || "").toLowerCase().trim();
    const typeFilter = document.getElementById("rechargeFilterType")?.value || "ALL";
    const statusFilter = document.getElementById("rechargeFilterStatus")?.value || "ALL";
    
    const filtered = recharges.filter(log => {
        const matchesQuery = query === "" || 
            log.custName.toLowerCase().includes(query) ||
            log.custID.toLowerCase().includes(query) ||
            log.custMobile.includes(query) ||
            log.transactionID.toLowerCase().includes(query) ||
            log.planName.toLowerCase().includes(query) ||
            (log.tenure && log.tenure.toLowerCase().includes(query));
        
        const matchesType = typeFilter === "ALL" || log.planCategory === typeFilter;
        const matchesStatus = statusFilter === "ALL" || log.status === statusFilter;
        
        return matchesQuery && matchesType && matchesStatus;
    });
    
    renderRechargeLogs(filtered);
}

function clearRechargeLogs() {
    if (confirm("Are you sure you want to clear all customer recharge database logs permanently?")) {
        recharges = [];
        localStorage.setItem("arb_recharges", JSON.stringify(recharges));
        renderRechargeLogs();
...
// ==========================================================================
// Owner Dashboard Tab 2: Broadband Plan CRUD Manager
// ==========================================================================
...
function renderAdminPlansList() {
    ...
    plans.forEach((plan, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="action-btn edit" onclick="startEditPlan(${index})" title="Edit Plan"><i class="fas fa-edit"></i></button>
                <button class="action-btn delete" onclick="deletePlan(${index})" title="Delete Plan"><i class="fas fa-trash-alt"></i></button>
            </td>
        `;
        tableBody.appendChild(row);
    });
...
function startEditPlan(index) {
    const plan = plans[index];
    
    document.getElementById("editPlanIndex").value = index;
    document.getElementById("planName").value = plan.name;
    document.getElementById("planSpeed").value = plan.speed;
    document.getElementById("planSpeedPercent").value = plan.speedPercent;
    document.getElementById("planLimit").value = plan.limit;
    document.getElementById("planFupSpeed").value = plan.fupSpeed;
    ...
    document.getElementById("cancelPlanEditBtn").style.display = "inline-block";
    
    document.querySelector(".plan-form-container").scrollIntoView({ behavior: 'smooth' });
}

function resetPlanEditor() {
    document.getElementById("planEditorForm").reset();
    document.getElementById("editPlanIndex").value = "-1";
    document.getElementById("planFormTitle").textContent = "Add New Broadband Plan";
    document.getElementById("savePlanBtn").textContent = "Save Plan";
    document.getElementById("cancelPlanEditBtn").style.display = "none";
}

function handlePlanEditorSubmit(e) {
    e.preventDefault();
    
    const index = parseInt(document.getElementById("editPlanIndex").value);
    const name = document.getElementById("planName").value.trim();
    const speed = document.getElementById("planSpeed").value.trim();
    const speedPercent = parseInt(document.getElementById("planSpeedPercent").value);
    const limit = document.getElementById("planLimit").value.trim();
    const fupSpeed = document.getElementById("planFupSpeed").value.trim();
    ...
    const planData = { 
        ...
        fupSpeed, 
        price, 
        price3M: Math.round(price * 3 * 0.95),
        price6M: Math.round(price * 6 * 0.90),
        price12M: price * 10,
        validity,
        popular: false
    };
    ...
    if (index === -1) {
        plans.push(planData);
        showToast("New broadband plan added successfully!", "success");
    } else {
...
function deletePlan(index) {
    ...
    if (confirm(`Are you sure you want to delete "${plan.name}"?`)) {
        ...
        renderAdminPlansList();
        renderCartPage();
        showToast(`Deleted plan "${plan.name}"`, "warning");
    }
}

// ==========================================================================
// Owner Dashboard Tab 2.5: OTT Plans CRUD Manager
// ==========================================================================

function autoCalculateOttPrices() {
    const price1M = parseInt(document.getElementById("ottPrice1M").value) || 0;
    if (price1M > 0) {
        document.getElementById("ottPrice3M").value = Math.round(price1M * 3 * 0.95);
        document.getElementById("ottPrice6M").value = Math.round(price1M * 6 * 0.90);
        document.getElementById("ottPrice12M").value = price1M * 10;
    }
}

function renderAdminOttPlansList() {
    const tableBody = document.getElementById("adminOttTableBody");
    if (!tableBody) return;
    
    tableBody.innerHTML = "";
    
    if (ottPlans.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding: 20px;">No OTT bundle plans found.</td></tr>`;
        return;
    }
    
    ottPlans.forEach((plan, index) => {
        const row = document.createElement("tr");
        const appsPreview = (plan.ottApps || []).slice(0, 4).join(", ") + ((plan.ottApps || []).length > 4 ? ` (+${(plan.ottApps || []).length - 4})` : '');
        const specsText = [plan.speed, plan.limit].filter(Boolean).join(" • ");
        
        row.innerHTML = `
            <td>
                <strong>${plan.name}</strong>
                ${specsText ? `<div style="font-size:12px; color:#64748b;">${specsText}</div>` : ''}
            </td>
            <td>
                <span class="ott-app-count-tag" style="margin-bottom:4px; display:inline-block;">${(plan.ottApps || []).length} Apps</span>
                <div style="font-size:11px; color:#334155;">${appsPreview}</div>
            </td>
            <td><strong>₹${plan.price1M}</strong></td>
            <td>₹${plan.price3M}</td>
            <td>₹${plan.price6M}</td>
            <td><strong style="color:var(--primary-color);">₹${plan.price12M}</strong></td>
            <td>
                <button class="action-btn edit" onclick="startEditOttPlan(${index})" title="Edit OTT Plan"><i class="fas fa-edit"></i></button>
                <button class="action-btn delete" onclick="deleteOttPlan(${index})" title="Delete OTT Plan"><i class="fas fa-trash-alt"></i></button>
            </td>
        `;
        tableBody.appendChild(row);
    });
...
function startEditOttPlan(index) {
    const plan = ottPlans[index];
    
    document.getElementById("editOttIndex").value = index;
    document.getElementById("ottPlanName").value = plan.name;
    document.getElementById("ottPlanBadge").value = plan.badge || "";
    document.getElementById("ottPrice1M").value = plan.price1M;
    document.getElementById("ottPrice3M").value = plan.price3M;
    document.getElementById("ottPrice6M").value = plan.price6M;
    document.getElementById("ottPrice12M").value = plan.price12M;
    document.getElementById("ottPlanDesc").value = plan.description || "";
    
    const checkboxes = document.querySelectorAll('input[name="ottAppCheck"]');
    const knownApps = plan.ottApps || [];
    const customList = [];
    
    checkboxes.forEach(cb => {
        if (knownApps.includes(cb.value)) {
            cb.checked = true;
        } else {
            cb.checked = false;
        }
    ...
    knownApps.forEach(app => {
        if (!standardValues.includes(app)) {
            customList.push(app);
        }
    });
    document.getElementById("ottCustomApps").value = customList.join(", ");
    
    document.getElementById("ottFormTitle").innerHTML = `<i class="fas fa-edit"></i> Edit OTT Plan: ${plan.name}`;
    document.getElementById("saveOttPlanBtn").innerHTML = `<i class="fas fa-save"></i> Update OTT Plan`;
    document.getElementById("cancelOttEditBtn").style.display = "inline-block";
    
    document.getElementById("dash-ott-tab").scrollIntoView({ behavior: 'smooth' });
}

function resetOttEditor() {
    document.getElementById("ottEditorForm").reset();
    document.getElementById("editOttIndex").value = "-1";
    document.getElementById("ottFormTitle").innerHTML = `<i class="fas fa-tv"></i> Add New OTT Bundle Plan`;
    document.getElementById("saveOttPlanBtn").innerHTML = `<i class="fas fa-save"></i> Save OTT Plan`;
    document.getElementById("cancelOttEditBtn").style.display = "none";
}

function handleOttEditorSubmit(e) {
    e.preventDefault();
    
    const index = parseInt(document.getElementById("editOttIndex").value);
    const name = document.getElementById("ottPlanName").value.trim();
    const badge = document.getElementById("ottPlanBadge").value.trim();
    
    const existingPlan = index !== -1 ? ottPlans[index] : {};
    const speed = existingPlan.speed || "";
    const limit = existingPlan.limit || "";
    const fupSpeed = existingPlan.fupSpeed || "";
    
    const price1M = parseInt(document.getElementById("ottPrice1M").value);
    const price3M = parseInt(document.getElementById("ottPrice3M").value);
    const price6M = parseInt(document.getElementById("ottPrice6M").value);
    const price12M = parseInt(document.getElementById("ottPrice12M").value);
    const description = document.getElementById("ottPlanDesc").value.trim();
    
    const selectedApps = [];
    document.querySelectorAll('input[name="ottAppCheck"]:checked').forEach(cb => {
        selectedApps.push(cb.value);
    });

    const customAppsStr = document.getElementById("ottCustomApps").value.trim();
    if (customAppsStr) {
        customAppsStr.split(",").forEach(item => {
            const clean = item.trim();
            if (clean && !selectedApps.includes(clean)) {
                selectedApps.push(clean);
            }
        });
    }
    ...
    const ottPlanData = {
        ...
        price6M,
        price12M,
        ottApps: selectedApps,
        description
    };
    
    if (index === -1) {
        ottPlans.push(ottPlanData);
        showToast("New OTT Bundle plan added successfully!", "success");
    } else {
        ottPlans[index] = ottPlanData;
        showToast(`OTT Plan "${name}" updated successfully!`, "success");
    }
...
function deleteOttPlan(index) {
    const plan = ottPlans[index];
    if (confirm(`Are you sure you want to delete OTT Plan "${plan.name}"?`)) {
        ottPlans.splice(index, 1);
        localStorage.setItem("arb_ott_plans", JSON.stringify(ottPlans));
        renderOttPlans();
        populateRechargeDropdown();
        updateRechargeFormPrices();
        renderAdminOttPlansList();
        renderCartPage();
        showToast(`Deleted OTT plan "${plan.name}"`, "warning");
    }
}

// ==========================================================================
// Owner Dashboard Tab 3: Contact Messages
// ==========================================================================

function renderContactMessages(filtered = null) {
    const tableBody = document.getElementById("contactMessagesTableBody");
    if (!tableBody) return;
    
    tableBody.innerHTML = "";
    const dataset = filtered || contactMessages;
    
    if (dataset.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding: 20px;">No messages found.</td></tr>`;
        return;
    }
    ...
    dataset.forEach((msg) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <div>${msg.mobile}</div>
                <div style="font-size:11px; color:#888;">${msg.email}</div>
            </td>
            <td><strong>${msg.subject}</strong></td>
            <td style="max-width: 300px; font-size: 12px;">${msg.message}</td>
            <td>
                <button class="action-btn delete" onclick="deleteContactMessage(${msg.id})" title="Delete"><i class="fas fa-trash-alt"></i></button>
            </td>
        `;
        tableBody.appendChild(row);
    });
...
function clearContactMessages() {
    if (confirm("Are you sure you want to delete all contact messages?")) {
        ...
        localStorage.setItem("arb_contact_messages", JSON.stringify(contactMessages));
        renderContactMessages();
        showToast("All messages deleted", "warning");
    }
}

function filterContactMessages() {
    const q = (document.getElementById("contactMessageSearch")?.value || "").toLowerCase().trim();
    if (!q) {
        renderContactMessages();
        return;
    }
...
// ==========================================================================
// Contact & Location Settings Manager
// ==========================================================================

function populateSettingsForm() {
    const s = contactSettings;
    
    document.getElementById("settingsAdminUser").value = localStorage.getItem("arb_admin_user") || "admin";
    document.getElementById("settingsAdminPass").value = localStorage.getItem("arb_admin_pass") || "admin123";
    ...
    document.getElementById("settingsAltEmail").value = s.altEmail || "";
    document.getElementById("settingsCustCare").value = s.customerCare || "";
    document.getElementById("settingsEmergency").value = s.emergencySupport || "";
    document.getElementById("settingsAddress").value = s.address || "";
    document.getElementById("settingsCity").value = s.city || "";
    document.getElementById("settingsState").value = s.state || "";
    document.getElementById("settingsPinCode").value = s.pinCode || "";
    document.getElementById("settingsServiceArea").value = s.serviceArea || "";
    document.getElementById("settingsMapEmbed").value = s.mapEmbed || "";
    document.getElementById("settingsLat").value = s.lat || "";
    document.getElementById("settingsLng").value = s.lng || "";
    document.getElementById("settingsWorkingDays").value = s.workingDays || "";
    document.getElementById("settingsOpeningTime").value = s.openingTime || "";
    document.getElementById("settingsClosingTime").value = s.closingTime || "";
    document.getElementById("settingsHolidayMessage").value = s.holidayMessage || "";
    
    document.getElementById("togglePhone").checked = s.showPhone !== false;
    document.getElementById("toggleWhatsapp").checked = s.showWhatsapp !== false;
    document.getElementById("toggleEmail").checked = s.showEmail !== false;
    document.getElementById("toggleLocation").checked = s.showLocation !== false;
    document.getElementById("toggleHours").checked = s.showHours !== false;
    document.getElementById("toggleMap").checked = s.showMap !== false;
...
function handleSettingsSave(e) {
    ...
    const adminUser = document.getElementById("settingsAdminUser").value.trim();
    const adminPass = document.getElementById("settingsAdminPass").value.trim();
    if (adminUser) localStorage.setItem("arb_admin_user", adminUser);
    if (adminPass) localStorage.setItem("arb_admin_pass", adminPass);
    
    contactSettings = {
        phone: document.getElementById("settingsPhone").value.trim(),
        whatsapp: document.getElementById("settingsWhatsapp").value.trim(),
        email: document.getElementById("settingsEmail").value.trim(),
        altEmail: document.getElementById("settingsAltEmail").value.trim(),
        customerCare: document.getElementById("settingsCustCare").value.trim(),
        emergencySupport: document.getElementById("settingsEmergency").value.trim(),
        ...
        serviceArea: document.getElementById("settingsServiceArea").value.trim(),
        mapEmbed: document.getElementById("settingsMapEmbed").value.trim(),
        lat: document.getElementById("settingsLat").value.trim(),
        lng: document.getElementById("settingsLng").value.trim(),
        workingDays: document.getElementById("settingsWorkingDays").value.trim(),
        openingTime: document.getElementById("settingsOpeningTime").value.trim(),
        closingTime: document.getElementById("settingsClosingTime").value.trim(),
        holidayMessage: document.getElementById("settingsHolidayMessage").value.trim(),
        showPhone: document.getElementById("togglePhone").checked,
        showWhatsapp: document.getElementById("toggleWhatsapp").checked,
        showEmail: document.getElementById("toggleEmail").checked,
        showLocation: document.getElementById("toggleLocation").checked,
        showHours: document.getElementById("toggleHours").checked,
        showMap: document.getElementById("toggleMap").checked
    };
    
    localStorage.setItem("arb_contact_settings", JSON.stringify(contactSettings));
    renderContactPage();
    triggerSettingsPreview();
    showToast("Contact and company settings saved live!", "success");
}

function triggerSettingsPreview() {
    const previewContainer = document.getElementById("liveSettingsPreviewCard");
    if (!previewContainer) return;
    
    const s = contactSettings;
    previewContainer.innerHTML = `
        <div style="font-weight:700; font-size:14px; margin-bottom:12px; color:var(--dark-bg);">
            <i class="fas fa-map-marker-alt" style="color:var(--primary-color);"></i> ${s.address}, ${s.city} - ${s.pinCode}
        </div>
        <div style="font-size:12px; color:var(--text-muted); margin-bottom:10px;">
            <strong>Phone:</strong> ${s.phone} | <strong>WhatsApp:</strong> ${s.whatsapp}
        </div>
        <div style="font-size:12px; color:var(--text-muted); margin-bottom:10px;">
            <strong>Hours:</strong> ${s.workingDays} (${s.openingTime} - ${s.closingTime})
        </div>
        <div style="font-size:11px; background:#e0f2fe; color:#0369a1; padding:8px; border-radius:4px;">
            <i class="fas fa-info-circle"></i> Service Coverage: ${s.serviceArea}
        </div>
    `;
}
